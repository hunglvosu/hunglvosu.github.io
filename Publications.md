...
- **Spanners in Planar Domains via Steiner Spanners and non-Steiner Tree Covers**
  ...
  <details><summary style="color:#7C4700">Abstract</summary>
  <font color = "7C4700">
  Euclidean spanners are important geometric objects that have been extensively studied since the 1980s. The two most basic "compactness" measures of a Euclidean spanner $E$ are the size (number of edges) $|E|$ and the weight (sum of edge weights) $\|E\|$. The state-of-the-art constructions of Euclidean $(1+\epsilon)$-spanners in $\mathbb{R}^d$ have $O_d\brac{n\cdot \epsilon^{-d+1}}$ edges (or sparsity $O_d(\epsilon^{-d+1})$) and weight $O_d\brac{\epsilon^{-d}\log\epsilon^{-1}} \cdot \|E_\mathrm{mst}\|$  (or lightness $O_d(\epsilon^{-d}\log\epsilon^{-1})$); here $O_d$ suppresses a factor of $d^{O(d)}$ and  $\|E_\mathrm{mst}\|$ denotes the weight of a minimum spanning tree of the input point set.<br><br>  
